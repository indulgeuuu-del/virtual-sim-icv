...
本次运行将第三方配置放到本地工作目录，并设置离线模式；需要复用时可在当前 PowerShell 执行（不更改系统设置）：
...
最后回看标注视频，人工记录每类/每方向的真实数量再比较。二次NMS仍跨类别处理、目标关联仍依赖输入顺序，边界与方向规则也尚未按本届场景校准。
...
结论：视频到输出文件的技术流程已恢复，真实识别与计数质量尚未恢复。当前样本与比赛画面不同，模型泛化不足是待验证因素；零计数也可能涉及短轨迹、目标关联、边界和方向条件，尚未逐项归因。下一阶段应记录每条轨迹为何计入/未计入，结合适合的场景视频和人工真值定位问题，不直接凭这个视频调参或重训。
