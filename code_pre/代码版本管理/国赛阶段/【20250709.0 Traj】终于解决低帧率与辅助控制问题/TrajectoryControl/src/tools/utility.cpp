...
// 将 SSD::SimPoint3DVector 转换为 std::vector<cv::Point2f>
std::vector<cv::Point2f> convertToCvPoint2fVector(const SSD::SimPoint3DVector& simPoints) {
    std::vector<cv::Point2f> cvPoints;
    for (const auto& simPoint : simPoints) {
        // 假设我们只需要 x 和 y 坐标作为 2D 点
        cvPoints.push_back(cv::Point2f(simPoint.x, simPoint.y));
    }
    return cvPoints;
}
