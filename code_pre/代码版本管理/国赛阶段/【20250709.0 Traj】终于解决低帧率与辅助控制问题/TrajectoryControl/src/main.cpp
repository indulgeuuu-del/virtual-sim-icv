...
int main(int argc, char* argv[]) 
{
	...
	if (argc >= 2 && (strcmp(argv[1], "draw") == 0)) ASSERT(main_draw() == 0, "主函数 main_draw 意外退出");
	else if (argc >= 2 && (strcmp(argv[1], "obtain") == 0)) ASSERT(main_obtain() == 0, "主函数 main_obtain 意外退出");
...
void loadJson(void)
{
	...
	/* 解析预设规则集 */
	RuleSet ruleSet;
	if (ruleSet.loadFromYamlFile(SIMONE_ADAS_DIR + "TrajectoryControl/m_strategy/global.yaml")) {
